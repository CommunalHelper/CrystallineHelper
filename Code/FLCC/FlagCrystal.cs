...
{
    [Tracked]
    [CustomEntity("FlushelineCollab/FlagCrystal")]
    public class FlagCrystal : Actor
    {
        ...
        {
            if (FlushelineModule.DJMapHelperLoaded)
                UpdateTheoBarriers(true);
            ...
            if (isTheo && Scene.Tracker.GetEntities<FacingBlock>().Any(e => (e as FacingBlock).ActorType == FacingBlock.FacingActorTypes.Theo)) {
                if (Hold.IsHeld)
                    Facing = Hold.Holder.Facing;
                else if (Speed.X > 0)
                    Facing = Facings.Right;
                else
                    Facing = Facings.Left;

                backSprite.Scale.X = theoSprite.Scale.X = frontSprite.Scale.X = (int)Facing;
            }

            if (FlushelineModule.DJMapHelperLoaded)
                UpdateTheoBarriers(false);
        }
        ...
        private void UpdateTheoBarriers(bool collidable) {
            foreach (var barrier in Scene.Tracker.GetEntities<TheoCrystalBarrier>())
                barrier.Collidable = collidable;
        }
