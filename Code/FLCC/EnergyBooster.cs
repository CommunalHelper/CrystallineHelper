...
{
	[CustomEntity("FlushelineCollab/EnergyBooster")]
    public class EnergyBooster : Entity
    {
		...
		private void OnPlayer(Player player)
		{
			if (respawnTimer <= 0f && cannotUseTimer <= 0f && BoostingPlayer == null)
			{
				...
				playerData.Set("inEnergyBooster", this);
		...
		public void PlayerReleased(Player player = null)
		{
			...
			{
				DynData<Player> playerData = new DynData<Player>(player);
				EnergyBooster currentBooster = playerData.Get<EnergyBooster>("inEnergyBooster");
				if (currentBooster == this)
					playerData.Set<EnergyBooster>("inEnergyBooster", null);
				else
		...
		private static void Player_CallDashEvents(On.Celeste.Player.orig_CallDashEvents orig, Player self)
		{
			...
			{
				var booster = playerData.Get<EnergyBooster>("inEnergyBooster");
				if (booster != null)
				{
		...
		private static PlayerDeadBody Player_Die(On.Celeste.Player.orig_Die orig, Player self, Vector2 direction, bool evenIfInvincible, bool registerDeathInStats)
		{
			...
			{
				DynData<Player> playerData = new DynData<Player>(self);
				EnergyBooster booster = playerData.Get<EnergyBooster>("inEnergyBooster");
				if (booster != null)
					booster.PlayerDied();
			}
			return body;
